...
import type { AgePatientsResponse, SortState } from '../../types/report';

interface AgePatientsPanelProps {
  ageGroup: string;
  data: AgePatientsResponse;
  pageSize: number;
  sort: SortState;
  loading: boolean;
...
export function AgePatientsPanel({ ageGroup, data, pageSize, sort, loading, onSortChange, onPageChange, onPageSizeChange }: AgePatientsPanelProps) {
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 bg-blue-50 px-4 py-3">
        <h2 className="text-base font-semibold text-blue-950">รายชื่อผู้ป่วยช่วงอายุ {ageGroup} ปี</h2>
        <p className="mt-1 text-sm text-blue-800">แสดงรายชื่อผู้ป่วยจากแท่งกราฟช่วงอายุที่เลือก ตามตัวกรองปัจจุบัน</p>
      </div>
        ...
        <table className="min-w-full divide-y divide-slate-200">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {!loading &&
              data.items.map((patient) => (
                <tr key={`${patient.hn}-${patient.patient_name}`} className="hover:bg-slate-50">
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-slate-700">{patient.age_years.toLocaleString('th-TH')}</td>
            ...
            {!loading && data.items.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-sm text-slate-500">
                  ไม่พบรายชื่อผู้ป่วยในช่วงอายุนี้
                </td>
              </tr>
            ) : null}
          </tbody>
